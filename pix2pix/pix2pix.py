...
class pix2pix:
    def __init__(self,
        ...
        # discriminate
        self.D_W1 = tf.Variable(tf.truncated_normal([4, 4, 6, 64],stddev=0.02), name='D_W1')
    ...
    def generate(self,img):
        ...
        layer_10 = tf.nn.conv2d_transpose(layer_9,
                                         self.G_W10,
                                         output_shape=[self.batch_size, 4, 4, 512],
                                         strides=[1, 2, 2, 1])
    ...
    def sample_generate(self,img,img_shape,sess):
        # 256x256x3 -> 128x128x64
        # padding
        # SAME : appropriate pad
        # VALID : zero pad
        input_img = tf.placeholder(tf.float32, [self.batch_size] + img_shape)
        ...
        samples = sess.run(output_,feed_dict={input_img: img})
